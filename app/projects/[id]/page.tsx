...
type Project = {
  id: number;
  image: StaticImageData | string; // supports imported images or URL strings
  alt: string;
  name: string;
  description?: string;           // optional if missing
  overview: Array<{
    number: string;
    name: string;
    desc: string | string[];
...
const ProjectDetailPage = () => {
  ...
  const projectDetails = [
    { title: "Service", description: "3D & Cut - Out" },
    { title: "Location", description: "Tamil Nadu" },
  ...
  useEffect(() => {
    if (params.id) {
      ...
      if (foundProject) {
        ...
        if (category) setProjectCategory(category.category);
      }
    }
  }, [params.id]);
          ...
          <div className="absolute top-0 w-full md:h-[40%] h-[25%] flex flex-col items-center justify-center z-10 px-3 xs:px-4 sm:px-6 md:px-8 lg:px-12">
            <div className="text-center space-y-0.5 xs:space-y-1 sm:space-y-2 md:space-y-3 lg:space-y-4">
              {altWords.map((word, index) => (
                <h1
                  key={index}
                  className={`text-2xl xs:text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl 2xl:text-8xl leading-[0.8] xs:leading-[0.82] sm:leading-[0.85] md:leading-[0.87] lg:leading-[0.9] uppercase font-bold transition-all duration-300 ease-out max-w-full break-words text-white`}
      ...
      <div className="w-full mx-auto px-4 sm:px-6 lg:px-10 mt-20 mb-20">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8 max-w-6xl mx-auto">
          {project.contents.map((item, index) => (
            <div
              key={index}
              className="group flex flex-col items-center text-center justify-center p-8 border-[#E7E5E5] rounded-2xl border transition-all duration-300 ease-in-out hover:border-[#EC2B45] hover:shadow-lg hover:shadow-[#EC2B45]/20 hover:-translate-y-1 hover:scale-105 bg-white"
            >
